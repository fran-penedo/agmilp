...
class PlotAssumptionMinining(object):
    ...
    def plot_step(self, traces, sat_for):
        ...
        sc = self.ax.scatter(
            x[-1], y[-1], marker="s", s=(plt.rcParams["lines.markersize"] ** 2) * 2
        )
...
def compute_rectangles(formula):
    if isinstance(formula, stl.STLPred):
        ...
        if expr.index > 1:
            raise ValueError(
                "Can only compute 2D rectangles, given dimension index {}".format(
                    expr.index
                )
            )
    ...
    elif isinstance(formula, stl.STLNot):
        rects = compute_rectangles(formula.args[0])
        if len(rects) != 1:
            raise ValueError(
                "Cannot compute rectangles for formula: NOT can only precede a half space"
            )
    ...
    elif isinstance(formula, stl.STLAnd):
        rects_list = [compute_rectangles(f) for f in formula.args]
        return np.array(
            [intersect_rects(np.array(rects)) for rects in it.product(*rects_list)]
        )

    elif isinstance(formula, stl.STLOr):
        return np.array(
            [
                rect
                for rects in [compute_rectangles(f) for f in formula.args]
                for rect in rects
            ]
        )
